# Script to remove non-essential comments from TypeScript files while preserving important ones

$projectRoot = "D:\logistrack"
...
# Get TypeScript files but exclude the ones in excluded directories
...
$tsxFiles = Get-ChildItem -Path $projectRoot -Filter "*.tsx" -Recurse | Where-Object {
    $filePath = $_.FullName
    $shouldInclude = $true
...
function Is-ImportantComment {
    ...
    # Clean JSX comment markers for proper pattern matching
    $cleanedText = $commentText -replace '\{\s*/\*|\*/\s*\}', ''
    
    $patterns = @(
        'godoc',
        'swagger:',
        '@\w+',        # TypeScript decorators and JSDoc tags
        '\+\w+',
        'TODO:',
        'FIXME:',
        'NOTE:',
        ...
        'tslint',      # TSLint directives
        'nosonar',     # SonarQube directives
        'istanbul',    # Istanbul coverage directives
        'pragma',      # TypeScript pragma comments
        'ts-ignore',   # TypeScript ignore comments
        'ts-expect-error',
        'ts-nocheck',
        'react-'       # React specific comments
    )
...
# Important file patterns to avoid modifying
...
foreach ($file in $files) {
    # Skip important configuration and setup files
    ...
    # Check for signs this might be a generated file or library code
    ...
    for ($lineIndex = 0; $lineIndex -lt $lines.Length; $lineIndex++) {
        ...
        # Handle standard single-line comments
        if ($line -match '^\s*//') {
        ...
        # Handle JSX single-line comments like {/* Comment */}
        if ($line -match '\{\s*/\*.*?\*/\s*\}') {
        ...
        # Handle JSX multi-line comments starting with {/*
        ...
        # Handle inline comments for non-JSX styles
        $commentPos = -1
        $inString = $false
        $stringChar = ''
        $escape = $false
